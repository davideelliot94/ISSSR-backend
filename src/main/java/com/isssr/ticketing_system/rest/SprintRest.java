...
//    @JsonView(JsonViews.Basic.class)
     ...
     */
    @JsonView(JsonViews.Basic.class)
    @RequestMapping(path = "product/{id}/visualize", method = RequestMethod.GET)
    public ResponseEntity getSprintProduct(@PathVariable Long id) {
        List<SprintDTO> sprints;
        try {
            sprints = sprintCreateController.getSprintsByProduct(id);
        } catch (Exception e) {
            e.printStackTrace();
